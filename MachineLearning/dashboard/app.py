...
def inference(file_path, class_names):
    ...
    df = pd.DataFrame({
        ...
        "Persentase Prediksi (%)": (pred * 100).round(2)
    }).sort_values("Persentase Prediksi (%)", ascending=False).reset_index(drop=True)
...
with tabs[2]:
    ...
    if fetch_button and url:
        try:
            ...
            st.audio(file_path, format=content_type if content_type.startswith("audio") else "audio/mp3")
